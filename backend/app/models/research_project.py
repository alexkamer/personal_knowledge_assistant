...
class ResearchProject(Base, UUIDMixin, TimestampMixin):
    ...
    goal: Mapped[str] = mapped_column(Text, nullable=False)  # Main research objective
    ...
    auto_generate_tasks: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    ...
    default_depth: Mapped[str] = mapped_column(String(20), default="thorough", nullable=False)
    # Use JSON instead of ARRAY for SQLite compatibility in tests
    default_source_types: Mapped[Optional[List[str]]] = mapped_column(JSON, nullable=True)
